...
class BodyUploadFileUploadPost:
    ...
    def to_multipart(self) -> types.RequestFiles:
        ...
        if isinstance(self.key, str) and self.key:
            effective_filename = self.key.rsplit("/", maxsplit=1)[-1] or effective_filename
        ...
        file_payload = self.file if isinstance(self.file, bytes) else str(self.file).encode()

        files.append(("file", (effective_filename, file_payload, effective_content_type)))
